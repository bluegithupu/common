...
return result;	
	 ...
	 */
	 @Override
	public String  getTeacherNickNameById(Long id)throws ServiceException, ServiceDaoException{
		
		      if(log.isInfoEnabled()){
	  log.info(" get nickName by id  : " + id );
	  }
	...
	try {
        ...
        nickName = (String) dao.getMapping("getTeacherNickNameById", new Object[] {id });
   } catch (DaoException e) {
			log.error(" get nickName wrong by id  : " + id );
			log.error(e);
			e.printStackTrace();
			throw new ServiceDaoException(e);
		}
  if(log.isInfoEnabled()){
   log.info(" get nickName success : " + nickName);
   }
		return nickName;
